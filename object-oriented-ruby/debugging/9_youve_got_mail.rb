...
# There is no custom `send` method defined for the `PostalService` object referenced by `johns_postal_service`, so the `send` method defined in `Object` is used instead. `send` invokes the method identified by its arguments, which in this case is the object referenced by `ellens_postal_service.street_address`: the string `'860 Blackbird Ln.'` Since Ruby thinks we are attempting to invoke a method, and no method by that name exists anywhere in the method lookup chain, we get a `NoMethodError`.

# If we want to keep line 91 as written, we need to include the `Mailing` module in the `PostalService` class definition. The module contains the functionality we are intending.